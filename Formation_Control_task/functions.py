            ...
            for node_j in range(n_agents):
                if node_j in list_neigh_i: # if the node j is a neighbor
                    if node_j != node_i: # if the nodes are not the sa,e
                        ...
                        B_temp[node_i,node_j] = np.sum([Pg_stack[node_i, k, :, :] for k in list_neigh_i])

        B = np.zeros((d*n_agents,d*n_agents))
        # another cycle to rebuild the matrix B
        for i in range(n_agents):
            for j in range(n_agents):
                for k in range(d):
                    for z in range(d):
                        B[i*d+k,j*d+z] = B_temp[i,j,k,z] # rebuild the matrix with the right shape
        return B
