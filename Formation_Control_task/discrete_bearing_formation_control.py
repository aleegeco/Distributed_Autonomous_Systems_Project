...
Node_pos = np.zeros((NN,d,1)) #it stores all the vector position for each node, it is a tensor with NN matrix of dimension (dxd),
...
Pg_stack = np.zeros((NN,NN,d,d)) # it stores all the matrices Pg_ij ( d x d ), it is a tensor of dimension 4 NN,NN of dimension (dxd)
