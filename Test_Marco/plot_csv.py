...
plt.semilogy(range(epochs-1), temp_sum_JJ[:-1])
plt.title("Sum of the cost function"); plt.grid()
plt.xlabel("iteration")
plt.ylabel("$\sum_i J_i$")
...
for k in range(epochs):
    temp_sum_norm[k] = np.sum(delta_u[k])
plt.semilogy(range(epochs-1), temp_sum_norm[:-1])
plt.title("Norm Delta"); plt.grid()
plt.xlabel("iteration")
